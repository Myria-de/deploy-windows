...
# Stand Dezember 2022: 5=Windows 10/11 Pro 1=Windows 10/11 Home
IMGIDX="--image-name=1"
...
UNATT="--unattend=$WORKDIR/deploy-win/unattend_x86.xml"
# Der Ordner, in dem die virtuelle Maschine erstellt wird
VMDIR=$WORKDIR/VMs
# Der Pfad und Name zur ISO-Datei
# für die Windows-Installation
ISOPATH=$WORKDIR/Win10_22H2_German_x32.iso
# Netwerkadapter konfigurieren
# Bezeichnung des Netzwerkadapters siehe "ip a" im Terminal
NICDEVICE=enp0s31f6
NICTYPE=bridged
# oder für NAT
# NICDEVICE= 
...
echo -e "- ${GREEN}Starte: $WORKDIR/deploy-win/setup_win10.py --disk=$NBDDEV --iso=$ISOPATH $IMGIDX $UEFI $UNATT $PPROC ${NC}"
...
echo -e "- ${GREEN}Device $NBDDEV für $IMAGEPATH trennen.${NC}"
sudo -E qemu-nbd --disconnect $NBDDEV
